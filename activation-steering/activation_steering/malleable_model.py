...
class MalleableModel(torch.nn.Module):
        ...
        for i in range(len(layers)):
            if not isinstance(layers[i], LeashLayer):
                if model.config.model_type == "qwen3":
                    attention_type = layers[i].attention_type
                layers[i] = LeashLayer(layers[i], i)
                if model.config.model_type == "qwen3":
                    layers[i].attention_type = attention_type
    ...
    def find_best_condition_point(self, positive_strings: List[str], negative_strings: List[str], condition_vector: 'SteeringVector', layer_range: Optional[Tuple[int, int]] = None, max_layers_to_combine: int = 1, threshold_range: Tuple[float, float] = (0.0, 1.0), threshold_step: float = 0.01, save_analysis: bool = False, file_path: Optional[str] = None, condition_threshold_comparison_mode: str = "mean", return_analysis: bool = False) -> Tuple[List[int], float, str, float]:
        ...
        if save_analysis:
            best_similarities = analysis_results[f"layer_{best_config[0][0]}"]["similarities"]
        ...
        if return_analysis:
            return best_config[0], rounded_threshold, best_config[2], best_f1, best_similarities
