import json
import sys
from argparse import ArgumentParser
from pathlib import Path
from typing import Any, Optional

from datasets import load_from_disk
from torch.utils.data import DataLoader
from transformers import DataCollatorWithPadding

sys.path.append(".")
from src.evaluation.templates import BASE_CHAT_TEMPLATE, CLASSIFIER_PROMPT_TEMPLATE
from src.inference.hidden_states import generate_hidden_states
from src.utils import load_model_for_pred


def get_train_thinking_dataloader(
    dataset_path: Path,
    responses_path: Path,
    tokenizer,
    prompt_template: Optional[str] = None,
    sample_size: Optional[int] = None,
    seed: Optional[Any] = None,
    prompt_only: bool = True,
    max_length: Optional[int] = None,
    **dataloader_kwargs,
):
    dataset = load_from_disk(dataset_path, keep_in_memory=True)
    with open(responses_path) as f:
        responses = json.load(f)
    responses = [example.split("</think>")[0] + "</think>" for example in responses]
    dataset.add_column("responses", responses)
    if sample_size is not None:
        dataset = dataset.shuffle(seed=seed)
        dataset = dataset.select(range(sample_size))

    def process_dataset(example):
        messages = example["messages"]
        if prompt_only:
            messages = messages[:1]
        thinking_text = (
            tokenizer.apply_chat_template(
                [
                    *messages,
                    {"role": "assistant", "content": example},
                ],
                tokenize=False,
            ).split("</think>")[0]
            + "</think>"
        )
        outputs = {
            "input_ids": tokenizer(
                thinking_text,
                truncation=max_length is not None,
                max_length=max_length,
            )["input_ids"]
        }
        outputs["labels"] = example["labels"]
        return outputs

    dataset = dataset.map(process_dataset).remove_columns(
        [x for x in dataset.column_names if x != "labels"]
    )
    collator = DataCollatorWithPadding(tokenizer=tokenizer, max_length=max_length)
    return DataLoader(dataset, collate_fn=collator, **dataloader_kwargs)


def get_eval_dataset_thinking_dataloader(
    dataset_path: Path,
    responses_path: Path,
    tokenizer,
    prompt_template: Optional[str] = None,
    sample_size: Optional[int] = None,
    seed: Optional[Any] = None,
    prompt_only: bool = True,
    max_length: Optional[int] = None,
    **dataloader_kwargs,
):
    dataset = load_from_disk(dataset_path, keep_in_memory=True)

    with open(responses_path) as f:
        responses = json.load(f)
    responses = [example.split("</think>")[0] + "</think>" for example in responses]
    dataset.add_column("responses", responses)
    if sample_size is not None:
        dataset = dataset.shuffle(seed=seed)
        dataset = dataset.select(range(sample_size))

    def process_dataset(example):
        if "messages" not in example:
            messages = [{"role": "user", "content": example["prompt"]}]
        else:
            messages = example["messages"]
        if prompt_only:
            messages = messages[:1]
        thinking_text = (
            tokenizer.apply_chat_template(
                [
                    *messages,
                    {"role": "assistant", "content": example},
                ],
                tokenize=False,
            ).split("</think>")[0]
            + "</think>"
        )
        outputs = {
            "input_ids": tokenizer(
                thinking_text,
                truncation=max_length is not None,
                max_length=max_length,
            )["input_ids"]
        }
        outputs["labels"] = float(example["prompt_harmfulness"])
        return outputs

    dataset = dataset.map(process_dataset).remove_columns(
        [x for x in dataset.column_names if x != "labels"]
    )
    collator = DataCollatorWithPadding(tokenizer=tokenizer, max_length=max_length)
    return DataLoader(dataset, collate_fn=collator, **dataloader_kwargs)


if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument(
        "--base_model",
        type=str,
        default="mistralai/Mistral-7B-Instruct-v0.3",
        help="Path to the base model",
    )
    parser.add_argument(
        "--adapter_path", type=str, default=None, help="Path to the adapter"
    )
    parser.add_argument(
        "--adapter_weight",
        type=float,
        default=None,
        help="Weight with which adapter will be added into base model weights.",
    )
    parser.add_argument(
        "--dataset_path",
        type=Path,
        default="data/wildjailbreak",
        help="Path to wildjailbreak dataset saved to disk",
    )
    parser.add_argument(
        "--thinking_responses_path",
        type=Path,
        default=None,
        help="Path to thinking model responses",
    )
    parser.add_argument(
        "--eval_dataset_path",
        type=Path,
        default=None,
        help="Path to eval dataset saved to disk",
    )
    parser.add_argument(
        "--eval_thinking_responses_path",
        type=Path,
        default=None,
        help="Path to eval thinking model responses",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=1,
    )
    parser.add_argument(
        "--save_folder",
        type=Path,
        default="wildjailbreak_hidden_states",
        help="Path to save predictions",
    )
    parser.add_argument(
        "--remove_files_if_exists",
        type=bool,
        default=True,
        help="Wheter to remove parquet files if they exists",
    )
    parser.add_argument(
        "--response_dataset",
        action="store_true",
        help="Whether to use only the prompt part of the input",
    )
    parser.add_argument(
        "--calculate_mean_hidden_states",
        action="store_true",
        help="Whether to calculate mean hidden states for each example",
    )
    parser.add_argument(
        "--max_length",
        type=int,
        default=None,
        help="Maximum length of the input sequences. If None, no truncation will be applied.",
    )
    args = parser.parse_args()
    args.save_folder.mkdir(parents=True, exist_ok=True)
    model, tokenizer = load_model_for_pred(
        args.base_model,
        args.adapter_path,
        attn_implementation=None,
    )
    if tokenizer.pad_token is None:
        if tokenizer.unk_token is not None:
            tokenizer.pad_token = tokenizer.unk_token
            tokenizer.pad_token_id = tokenizer.unk_token_id
        else:
            tokenizer.pad_token = tokenizer.eos_token
            tokenizer.pad_token_id = tokenizer.eos_token_id

    if tokenizer.chat_template is None:
        if args.base_model == "allenai/wildguard":
            tokenizer.chat_template = CLASSIFIER_PROMPT_TEMPLATE
        else:
            tokenizer.chat_template = BASE_CHAT_TEMPLATE

    if (
        args.eval_dataset_path is not None
        and args.eval_thinking_responses_path is not None
    ):
        cur_save_folder = args.save_folder / "eval/wildguardtest"
        cur_save_folder.mkdir(parents=True, exist_ok=True)
        eval_dataloader = get_eval_dataset_thinking_dataloader(
            dataset_path=args.eval_dataset_path,
            responses_path=args.eval_thinking_responses_path,
            tokenizer=tokenizer,
            batch_size=args.batch_size,
            prompt_only=not args.response_dataset,
            max_length=args.max_length,
        )
        generate_hidden_states(
            model=model,
            dataloader=eval_dataloader,
            save_folder=cur_save_folder,
            remove_files_if_exists=args.remove_files_if_exists,
            calculate_mean_hidden_states=args.calculate_mean_hidden_states,
        )

    cur_save_folder = args.save_folder / "train"
    cur_save_folder.mkdir(parents=True, exist_ok=True)
    dataloader = get_train_thinking_dataloader(
        dataset_path=args.dataset_path,
        responses_path=args.thinking_responses_path,
        tokenizer=tokenizer,
        batch_size=args.batch_size,
        prompt_only=not args.response_dataset,
        max_length=args.max_length,
    )
    generate_hidden_states(
        model=model,
        dataloader=dataloader,
        save_folder=cur_save_folder,
        remove_files_if_exists=args.remove_files_if_exists,
        calculate_mean_hidden_states=args.calculate_mean_hidden_states,
    )
