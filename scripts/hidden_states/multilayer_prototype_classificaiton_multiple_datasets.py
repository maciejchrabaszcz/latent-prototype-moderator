...
def main(
    ...
    all_wg_gda_params = {}
    all_aegis_gda_params = {}
    all_toxichat_gda_params = {}
    layers_to_process = [
        x
    ...
    for layer_file in tqdm(layers_to_process, desc="Processing layers"):
        ...
        inv = get_bayes_precision_estimate(
            aegis_cov, aegis_hidden_states.size(0), aegis_hidden_states.size(1)
        )
        ...
        all_aegis_gda_params[layer_file.name] = aegis_gda_params
        all_wg_aegis_train_preds[layer_file.name + "0"] = (
            get_gda_pred(wg_aegis_hidden_states, **wg_gda_params)
            .detach()
            .cpu()
            .numpy()[:, 1]
        )
        ...
        inv = get_bayes_precision_estimate(
            toxichat_cov, toxichat_hidden_states.size(0), toxichat_hidden_states.size(1)
        )
        ...
        all_toxichat_gda_params[layer_file.name] = toxichat_gda_params
        all_wg_toxichat_train_preds[layer_file.name + "0"] = (
            get_gda_pred(wg_toxichat_hidden_states, **wg_gda_params)
            .detach()
            .cpu()
            .numpy()[:, 1]
        )
        ...
        all_wg_aegis_toxichat_train_preds[layer_file.name + "0"] = (
            get_gda_pred(wg_aegis_toxichat_hidden_states, **wg_gda_params)
            .detach()
            .cpu()
            .numpy()[:, 1]
        )
        ...
        all_wg_aegis_toxichat_train_preds[layer_file.name + "2"] = (
            get_gda_pred(wg_aegis_toxichat_hidden_states, **toxichat_gda_params)
            .detach()
            .cpu()
            .numpy()[:, 1]
        )
    ...
    meta_models = [
        LogisticRegression(C=0.1, penalty="l1", solver="liblinear").fit(
    ...
    for i, selected_datasets in enumerate(combinations_of_datasets):
        ...
        cur_gda_params = [all_gda_params[j] for j in selected_datasets]
