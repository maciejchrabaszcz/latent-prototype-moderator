...
def process_benchmark(
    ...
    for C, meta_model in meta_models.items():
        C_save_path = save_path / f"C_{C}"
        C_save_path.mkdir(parents=True, exist_ok=True)
        meta_preds = meta_model.predict_proba(meta_df)[:, 1]
        all_scores = calculate_scores(meta_preds, labels.cpu().numpy())
        save_preds(meta_preds, labels, C_save_path)
        if verbose:
            print(f"Scores for {benchmark_path.name} with C={C}:")
            print(all_scores)
        save_scores(all_scores, C_save_path)
...
def main(
    ...
    for layer_file in tqdm(layers_to_process, desc="Processing layers"):
        ...
        all_gda_params[layer_file.name] = gda_params
        all_train_preds[layer_file.name] = (
            get_gda_pred(train_hidden_states, **gda_params).detach().cpu().numpy()
        )[:, 1]
    meta_df = pd.DataFrame(all_train_preds)

    C_values = [0.0001, 0.001, 0.005, 0.05, 0.01, 0.5, 1, 10]  # Predefined list of C values
    meta_models = {}
    for C in C_values:
        meta_model = LogisticRegression(C=C, penalty="l1", solver="liblinear")
        meta_model.fit(meta_df, train_labels.cpu().numpy())
        meta_models[C] = meta_model

        # Save coefficients
        C_save_folder = save_folder / f"C_{C}"
        C_save_folder.mkdir(parents=True, exist_ok=True)
        with open(
            C_save_folder / "meta_model_coefficients.json", "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
                    layer_id: coef
                    for layer_id, coef in zip(
                        meta_df.columns, meta_model.coef_[0].tolist()
                    )
                },
                f,
                indent=4,
